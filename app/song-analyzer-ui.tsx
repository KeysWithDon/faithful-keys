...
export default function SongAnalyzer() {
  ...
  useEffect(() => {
    ...
    }), Math.max(180, 60000 / Math.max(10, activeChart.bpm ?? 72)));
    return () => window.clearInterval(interval);
  ...
  if (activeChart) return <section className="song-analyzer analyzer-results" aria-label="Song Analyzer results">
    ...
    <div className="analyzer-meta"><label className="meta-wide">COMPOSER<input aria-label="Song composer" value={activeChart.artist ?? ""} onChange={event => updateChart(chart => ({ ...chart, artist: event.target.value || null }))}/></label>{editingPublishedName && <label className="meta-wide">STYLE<input aria-label="Standard style" value={activeChart.publishedStandard?.style ?? ""} onChange={event => updateChart(chart => ({ ...chart, publishedStandard: { ...(chart.publishedStandard ?? { originalName: editingPublishedName, sourceTitle: chart.title }), style: event.target.value } }))}/></label>}<label>KEY<select value={activeChart.key} onChange={event => updateChart(chart => ({ ...chart, key: event.target.value }))}>{KEYS.map(key => <option key={key}>{key}</option>)}</select></label><label>MODE<select value={activeChart.mode} onChange={event => updateChart(chart => ({ ...chart, mode: event.target.value as "major" | "minor" }))}><option value="major">Major</option><option value="minor">Minor</option></select></label><label>BPM<input aria-label="Song BPM" type="number" inputMode="numeric" min="10" max="250" value={activeChart.bpm ?? ""} placeholder="Review" onChange={event => updateChart(chart => ({ ...chart, bpm: event.target.value ? Number(event.target.value) : null }))}/></label><label>METER<select value={activeChart.timeSignature} onChange={event => updateChart(chart => ({ ...chart, timeSignature: event.target.value }))}>{["2/4", "3/4", "4/4", "6/8"].map(meter => <option key={meter}>{meter}</option>)}</select></label><button onClick={() => updateChart(chart => transposeSongChart(chart, -1))}>− transpose</button><button onClick={() => updateChart(chart => transposeSongChart(chart, 1))}>+ transpose</button><label className="analyzer-toggle"><input type="checkbox" checked={showNumbers} onChange={event => setShowNumbers(event.target.checked)}/><span/> Nashville</label><label className="analyzer-toggle"><input type="checkbox" checked={reviewOnly} onChange={event => setReviewOnly(event.target.checked)}/><span/> Review only</label></div>
  ...
  return <section className="song-analyzer analyzer-entry" aria-label="Song Analyzer">
    ...
    {(job.status === "queued" || job.status === "processing") && <div className="analyzer-processing" role="status" aria-live="polite"><div className="analyzer-processing-mark" aria-hidden="true">FK</div><div><span>{job.status === "queued" ? "Queued securely" : "Measuring performance rhythm"}</span><strong>{job.status === "queued" ? "Checking the chart, tempo, and permissions…" : "Aligning beats, starts, and durations to your tempo…"}</strong><p>The worker uses no video-detected chord or estimated tempo in the chart. Source media is deleted after timing analysis.</p><i><b style={{ width: `${job.progress}%` }}/></i></div><em>{job.progress}%</em></div>}
    <div className="chart-first-flow">
      <section className={`analyzer-stage ${referenceChart && tempoReady ? "complete" : "current"}`}><header><span>1</span><div><b>Upload the chord chart and set tempo</b><small>Harmony, section order, and the selected BPM become authoritative.</small></div>{referenceChart && <em>✓ {referenceChart.chartReference?.chordCount ?? 0} chords</em>}</header><div className="chart-import-grid"><label className="file-drop chart-drop"><input type="file" accept=".txt,.csv,.json,.cho,.pro,.chordpro,text/plain,text/csv,application/json" onChange={event => void importChartFile(event.target.files?.[0] ?? null)}/><b>{chartImporting ? "Reading chart…" : chartFile?.name ?? referenceChart?.chartReference?.fileName ?? "Choose chart file"}</b><span>Text, CSV, ChordPro, or exported Faithful Keys JSON</span></label><div className="chart-paste"><label>OR PASTE A CHART<textarea value={chartText} onChange={event => setChartText(event.target.value)} placeholder={'[Verse]\n| Cmaj7 | Am7 D7 | G7 | Cmaj7 |\n[Chorus]\n| Fmaj7 | G7 | Cmaj7 | Cmaj7 |'}/></label><button disabled={!chartText.trim()} onClick={importPastedChart}>Use pasted chart</button></div></div>{referenceChart && <><div className="chart-import-summary"><div><b>{referenceChart.title}</b><span>{referenceChart.sections.length} sections · {referenceChart.chartReference?.chordCount} chart chords · {referenceChart.key} {referenceChart.mode}</span><small>You may edit any OCR or transcription mistake after analysis, then lock the correction.</small></div></div><div className={`analysis-tempo-panel ${tempoReady ? "valid" : "invalid"}`}><div><span>STEP 1 TEMPO</span><b>Set the performance tempo</b><small>Type a whole-number tempo. Faithful Keys will keep every beat evenly spaced at this BPM.</small></div><label className="analysis-tempo"><input aria-label="Tempo for analysis" type="number" inputMode="numeric" min="10" max="250" step="1" value={referenceChart.bpm ?? ""} placeholder="BPM" onChange={event => setReferenceChart(chart => chart ? { ...chart, bpm: event.target.value === "" ? null : Number(event.target.value) } : chart)}/><strong>BPM</strong><small>{tempoReady ? `${selectedTempo} BPM locked for analysis` : "Enter 10–250"}</small></label></div></>}{chartImportError && <p className="analyzer-error">{chartImportError}</p>}</section>
      <section className={`analyzer-stage ${referenceChart && tempoReady ? "current" : "disabled"}`} aria-disabled={!referenceChart || !tempoReady}><header><span>2</span><div><b>Add video or audio for rhythm</b><small>Faithful Keys locates the performance pulse while keeping your Step 1 tempo fixed.</small></div></header><div className="analyzer-source-tabs"><button disabled={!referenceChart || !tempoReady || job.status === "queued" || job.status === "processing"} className={sourceType === "upload" ? "active" : ""} onClick={() => setSourceType("upload")}>Upload video or audio</button><button disabled={!referenceChart || !tempoReady || job.status === "queued" || job.status === "processing"} className={sourceType === "youtube" ? "active" : ""} onClick={() => setSourceType("youtube")}>Paste YouTube link</button></div><div className="analyzer-source-card">{sourceType === "upload" ? <label className="file-drop"><input disabled={!referenceChart || !tempoReady} type="file" accept="audio/*,video/mp4,video/quicktime,video/webm,.mp3,.wav,.m4a,.aac,.flac,.ogg,.mp4,.mov,.webm" onChange={event => setAudioFile(event.target.files?.[0] ?? null)}/><b>{audioFile ? audioFile.name : "Choose the performance"}</b><span>Audio or video · up to 100 MB</span></label> : <label className="youtube-input"><span>YOUTUBE PERFORMANCE LINK</span><input disabled={!referenceChart || !tempoReady} value={youtubeUrl} onChange={event => setYoutubeUrl(event.target.value)} placeholder="https://youtube.com/watch?v=…"/><small>The performance supplies rhythm only and is deleted after processing.</small></label>}<label className="permission-check"><input disabled={!referenceChart || !tempoReady} type="checkbox" checked={permissionConfirmed} onChange={event => setPermissionConfirmed(event.target.checked)}/><span>I own this media or have permission to analyze it. I understand source media is processed temporarily and is not retained or shared.</span></label><div className="analyzer-progress"><span>{job.status === "idle" ? referenceChart && tempoReady ? "CHART + TEMPO READY" : "STEP 1 REQUIRED" : job.status.toUpperCase()}</span><i><b style={{ width: `${job.progress}%` }}/></i><small>{job.error ?? (workspaceStatus === "ready" ? "Chart chords and selected tempo stay fixed; media supplies rhythmic placement only." : "Preparing your private device workspace…")}</small></div><button className="primary analyzer-start" disabled={!check.allowed || workspaceStatus === "starting" || job.status === "queued" || job.status === "processing"} onClick={startReviewChart}>Measure performance timing</button>{referenceChart && !check.allowed && <p className="analyzer-error">{check.error}</p>}</div></section>
    </div>
  </section>;
}
