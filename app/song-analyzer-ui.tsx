...
export default function SongAnalyzer() {
  ...
  const tempoReady = Number.isFinite(selectedTempo) && selectedTempo >= 30 && selectedTempo <= 200;
  const check = !referenceChart
    ? { allowed: false as const, error: "Upload or paste the chord chart first." }
    : !tempoReady
      ? { allowed: false as const, error: "Set the tempo between 30 and 200 BPM in Step 1." }
      : mediaCheck;
  ...
  if (activeChart) return <section className="song-analyzer analyzer-results" aria-label="Song Analyzer results">
    ...
    <div className="analyzer-meta"><label className="meta-wide">COMPOSER<input aria-label="Song composer" value={activeChart.artist ?? ""} onChange={event => updateChart(chart => ({ ...chart, artist: event.target.value || null }))}/></label>{editingPublishedName && <label className="meta-wide">STYLE<input aria-label="Standard style" value={activeChart.publishedStandard?.style ?? ""} onChange={event => updateChart(chart => ({ ...chart, publishedStandard: { ...(chart.publishedStandard ?? { originalName: editingPublishedName, sourceTitle: chart.title }), style: event.target.value } }))}/></label>}<label>KEY<select value={activeChart.key} onChange={event => updateChart(chart => ({ ...chart, key: event.target.value }))}>{KEYS.map(key => <option key={key}>{key}</option>)}</select></label><label>MODE<select value={activeChart.mode} onChange={event => updateChart(chart => ({ ...chart, mode: event.target.value as "major" | "minor" }))}><option value="major">Major</option><option value="minor">Minor</option></select></label><label>BPM<input aria-label="Song BPM" type="number" min="30" max="200" value={activeChart.bpm ?? ""} placeholder="Review" onChange={event => updateChart(chart => ({ ...chart, bpm: event.target.value ? Number(event.target.value) : null }))}/></label><label>METER<select value={activeChart.timeSignature} onChange={event => updateChart(chart => ({ ...chart, timeSignature: event.target.value }))}>{["2/4", "3/4", "4/4", "6/8"].map(meter => <option key={meter}>{meter}</option>)}</select></label><button onClick={() => updateChart(chart => transposeSongChart(chart, -1))}>− transpose</button><button onClick={() => updateChart(chart => transposeSongChart(chart, 1))}>+ transpose</button><label className="analyzer-toggle"><input type="checkbox" checked={showNumbers} onChange={event => setShowNumbers(event.target.checked)}/><span/> Nashville</label><label className="analyzer-toggle"><input type="checkbox" checked={reviewOnly} onChange={event => setReviewOnly(event.target.checked)}/><span/> Review only</label></div>
