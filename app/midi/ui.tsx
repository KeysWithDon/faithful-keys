...
export function MidiSetup() {
  ...
  return <div className="midi-setup"><button type="button" onClick={()=>setOpen(v=>!v)} aria-expanded={open}>MIDI Setup · {midi.status}</button>{open && <section className="midi-panel" aria-label="MIDI keyboard setup">
    ...
    <p>{native ? 'Plug in a USB MIDI keyboard, or pair a Bluetooth MIDI keyboard below. Then connect and choose your keyboard.' : 'USB keyboards and operating-system-paired Bluetooth MIDI devices appear here when your browser exposes them. No MIDI? Every lesson also works onscreen.'}</p>
    {native && <button type="button" onClick={()=>native.bluetooth()}>Pair Bluetooth keyboard</button>}
    ...
    <p>Pressed notes: {midi.active.join(', ') || 'None'}. {native ? 'Turn up your iPhone volume to hear your playing.' : 'Bluetooth pairing is managed by your device, not this page.'}</p>
