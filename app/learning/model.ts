...
export type DailyGoalMinutes = 10 | 15 | 30 | 45 | 60;
...
export type Profile = { version: 1; id: string; mode: 'explore' | 'guided'; goal: LearningGoal; placementDone: boolean; dailyGoalMinutes: DailyGoalMinutes; dailyPracticeDay: string; dailyPracticeMs: number; skills: Record<string, SkillRecord>; history: Session[]; lastSession: Session | null; updatedAt: number };
export const DAY = 86400000;
export const DAILY_GOALS: DailyGoalMinutes[] = [10, 15, 30, 45, 60];
export function dayKey(now = Date.now()): string { const d = new Date(now); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`; }
export function freshProfile(now = Date.now()): Profile { return { version: 1, id: `local-${now}`, mode: 'explore', goal: GOALS[0], placementDone: false, dailyGoalMinutes: 15, dailyPracticeDay: dayKey(now), dailyPracticeMs: 0, skills: {}, history: [], lastSession: null, updatedAt: now }; }
export function recordPracticeTime(p: Profile, milliseconds: number, now = Date.now()): Profile {
  const today = dayKey(now);
  const sameDay = p.dailyPracticeDay === today;
  return { ...p, dailyPracticeDay: today, dailyPracticeMs: (sameDay ? p.dailyPracticeMs : 0) + Math.max(0, Math.round(milliseconds)), updatedAt: now };
}
...
export function completeSession(p: Profile, session: Session, now = Date.now()): Profile {
  ...
  for (const id of new Set(session.attempts.map(a => a.skillId))) {
    ...
    // A total attempt count cannot identify this safely across multiple
    // sessions, so use the persisted session id as the streaming marker.
    const streamed = p.lastSession?.id === session.id && p.lastSession.attempts.length >= a.length;
    if (!streamed) {
      r.attempts += a.length;
      r.correct += a.filter(attempt => attempt.correct).length;
      r.exposures += a.length;
      r.responseMs += a.reduce((sum, attempt) => sum + Math.max(0, attempt.responseMs), 0);
    }
...
export const PLACEMENT_SKILLS = ['notes','geography','major-scales','major-triads','minor-triads','root-position','basic-progressions','inversions','diatonic','rhythm'];
export function generatePlacementPlan(): string[] { return PLACEMENT_SKILLS.flatMap(id => Array.from({length: 4}, () => id)); }
export function generateAssessmentPlan(target: string, kind: 'placement' | 'test-out'): string[] {
  return kind === 'placement' ? generatePlacementPlan() : Array.from({length: 12}, () => target);
}
export function generateSession(p: Profile, target: string, count = 28, rng = Math.random): string[] {
