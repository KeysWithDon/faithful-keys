...
export default function GuidedLearning({profile,update,reset,error,playNotes,stopAudio,onFeature}:Props) {
  ...
  const sessionRef=useRef<Session|null>(null), recent=useRef<string[]>([]);
  ...
  function start(target:string,kind:Session['kind']='practice') {
    ...
    const items=kind==='test-out'||kind==='placement'?Array(5).fill(target):generateSession(profile,target);
  ...
  function finish() {
    const active=sessionRef.current;if(!active)return;
    update(p=>completeSession(p,active));
  ...
  if(session&&skill) {
    ...
    if(!prerequisitesMet(profile,id)&&!profile.skills[id]?.unlocked) return <section className="guided-shell"><p>Locked. Complete the prerequisite first.</p><button onClick={()=>{setSession(null);sessionRef.current=null;}}>Guided dashboard</button></section>;
  ...
  return <section className="guided-shell" aria-labelledby="guided-title">
    ...
    {view==='home'&&<>
      {!profile.placementDone?<section className="learning-focus"><small>FIND YOUR STARTING POINT</small><h2>Let’s meet you where you are.</h2><p>Take short, five-question checkpoints. Each passed prerequisite opens the next branch. Experienced players can test out; beginners can start with notes.</p><button className="learning-primary" onClick={()=>start(next.skill.id,'placement')}>Start placement · {next.skill.title}</button><button onClick={()=>update(p=>({...p,placementDone:true}))}>Start curriculum here</button><p className="learning-small">End placement whenever you are ready. Only skills you demonstrate receive credit.</p></section>:<section className="learning-focus"><small>YOUR NEXT STEP</small><h2>{next.skill.title}</h2><p>{next.reason}</p><p>Keys: {(profile.skills[next.skill.id]?.exposures??0)>=8?'All 12 tonal centers':'C, F, B♭'} · Stage {next.skill.stage}</p><button className="learning-primary" onClick={()=>start(next.skill.id)}>CONTINUE LEARNING</button><button onClick={()=>start(next.skill.id,'test-out')}>Test out of {next.skill.title}</button></section>}
